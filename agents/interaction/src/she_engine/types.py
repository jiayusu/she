...
class TurnRequest:
    ...
    level: int = 1                       # Legacy alias for language_level (L0-L5)
    language_level: Optional[int] = None # Explicit curriculum difficulty (L0-L5)
    scaffold_level: Optional[int] = None # Director-owned output support (S0-S6)
    route_intent: str = ""               # 上游可强制路由；空则引擎自行分类
    ctx_bundle: Optional[CtxBundle] = None

    @classmethod
    def from_dict(cls, d: Dict[str, Any]) -> "TurnRequest":
        lv = d.get("language_level", d.get("level", 1))
        try:
            lv = int(lv)
        except (TypeError, ValueError):
            lv = 1
        scaffold = d.get("scaffold_level")
        try:
            scaffold = max(0, min(6, int(scaffold))) if scaffold is not None else None
        except (TypeError, ValueError):
            scaffold = None
        return cls(
            ...
            language_level=max(0, min(5, lv)) if "language_level" in d else None,
...
class MemoryWrite:
    ...
    key: str
    value: str
    # A single turn is evidence, not confirmed mastery. This metadata is kept
    # out of the legacy wire schema and consumed by the shared-state writer.
