...
class Engine:
    ...
    def handle_turn(self, req: TurnRequest) -> TurnResponse:
        ...
        if self._cfg.follow_request_level and req.level:
            self._level = max(0, min(5, int(req.level)))
        ...
        if not utterance:
            ...
            if verdict.downgraded:
                ...
                self._level_locked = True   # 降级后由引擎接管级别，防被上游顶回
        ...
        if self._cfg.follow_request_level:
            self._level = max(0, min(5, int(req.level)))
        elif not self._level_locked and req.level:
            self._level = max(0, min(5, int(req.level)))
