...
- Files: `route/src/types.ts`, `route/src/learning-director.ts`, `route/src/app.ts`, `route/src/server.ts`, `route/src/index.ts`, route docs.
- Verification: `Set-Location -LiteralPath 'A:\working\she\route'; npm run typecheck`; existing route suite (`npm test`) remains green.
