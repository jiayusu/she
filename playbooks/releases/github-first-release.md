...
1. `scripts/verify.ps1` passes every local platform-independent check.
2. `scripts/audit_repository.ps1` passes.
3. `git -C '.' diff --check` passes and the worktree is clean.
...
```powershell
git -C '.' remote add origin 'https://github.com/jiayusu/she.git'
git -C '.' push -u origin main
git -C '.' ls-remote --heads origin main
